...
//debug
if(!is_writable($config->path_root."/include/smarty/templates_c/")) {
...
if($sitenr==2) {
	$php_settings=array(
			...
			"magic_quotes_gpc"=>(false==true)?"_ON":"_OFF", #(ini_get('magic_quotes_gpc')=="0")?"off":"on",
...
if($sitenr==3) {
	...
	$include_dir=is_writable($config->path_root."/include/");
	$backup_dir=is_writable($config->path_root."/include/backup/");
	$files_dir=is_writable($config->path_root."/include/files/");
	$temp_dir=is_writable($config->path_root."/temp/");
	$templates_c_dir=is_writable($config->path_root."/include/smarty/templates_c/");
	$setupphp=is_writable($config->path_root."/");
